...
> **Status:** ✅ Phases 0–16, 19, 20, 21, Phase A + **OCR Phases B–E**, Deepening D1–D5, S6a–d, S7, S2, S10a–c, S9a, Priority 6 infra, **Priority 7 (Multi-Target Sheets Redundancy — 43 tests)**, **RAG Phase 1–5 ✅** (694 RAG tests), **Multi-Domain Phase 1 ✅** (37 tests), **P1-4 FSSAI re-ingest ✅** (15 tests), **Evaluation Framework ✅** (77 tests), **Benchmark v1.0 frozen ✅**, **Rust PyO3 Normalizers ✅**, **Remote Inference Layer ✅ + deployed (Modal)**, **LangGraph Agent Pipeline + M5 ✅** (75 tests), **FastAPI Gateway ✅** (50 tests), **Config seam** (`app/shared/config.py` `cfg` — §3.6, completed 2026-08-22), **Phase 15 Analytics ✅** (15 tests, 2026-08-22), **Phase 18 RBAC ✅** (44/44 pass, 2026-08-26), **Work Diary ✅** (28/28 pass, 2026-08-26), **Security close-out S10c+S2 ✅** (12/12 pass, 2026-08-26), **Redis/Celery ssl_cert_reqs fix ✅** (11/11 pass, 2026-08-26), **Case File Preview (TDD) ✅** (9 tests, 2026-08-26), **Adjudication Preview (TDD) ✅** (9 tests, 2026-08-26), **Phase 12 ✅** (Legal Validation Engine — 46 tests), **CI/CD gates G1–G14 ✅** (46/46 pass, 2026-08-23), **Pre-commit hook stack fixed** (2026-08-23), **Phase 6 deferred**, all implemented & verified. Total: **~1,970 tests**. Phases 17 ⚠️ pending (Supabase bridge, conflict resolution, sync-status UI). Phase 19 ✅ complete (AI Case Intelligence — evidence strength + readiness score; 12/12 tests pass, 2026-09-07). ENV-2/3/5/10 open.
...
- [x] **Phase 18 RBAC — Multi-User Roles (completed 2026-08-26).** Wired the previously dead `Role`/`user_roles` tables: roles **`admin`** + **`fso`**; central gate `app/__init__.py::enforce_rbac` driven by `ROLE_BLUEPRINTS` (`app/shared/rbac.py`, deny = flash + redirect to landing); record-level scoping via `scoped_officer_name()`/`case_visible_to_user()` across CaseFile (`food_safety_officer_name`), Adjudication (`food_safety_officer`), Inspection list/create, Work Diary (locked to bound officer), child routes (editor/xref/toc/renumber/export/regenerate/comments inherit parent visibility); create-routes force-stamp the bound officer server-side; legacy mismatched names fail closed. Users bound via nullable `users.fso_name` (migration `add_user_fso_name`); provisioning seam `app/auth/provisioning.py` (`create_fso_account` with 1:1 binding uniqueness, `seed_fso_users`) + extended admin Add User form (Role + FSO dropdowns) + bulk seed `scripts/seed_fso_users.py`; existing users backfilled to `admin`. Notepad blueprint registered (+ `note`/`note_evaluation` tables via `db.create_all`; add Alembic migration on next schema pass). Comments API `app/comments/` (read/add on visible cases; delete author/admin). Flash rendering added to `base.html` (flashes were never displayed anywhere); repaired corrupted Jinja in `inspection/list.html:17`. `tests/test_rbac.py` — **44/44 pass**; workdiary/backup-monitoring/route-collision/checklist suites green alongside.
- [x] **Security hardening close-out — S10c backup monitoring + S2 CSP-report collector (completed 2026-08-26).** `run_backup()` (`app/services/backup_coordinator.py`) now persists per-target outcomes to the `settings` table via `record_backup_result()` (`last_backup_at` + `last_backup_results`); new public `GET /health/backups` dead-man's-switch (200 fresh/all-ok; 503 never / stale >26h / degraded) for uptime-monitor alerting; new public CSRF-exempt `POST /csp-report` collector filling the previously dangling Talisman `report_uri` (accepts `{"csp-report": …}` and `{"csp-violation-report": …}`, bounded 4 KB body, logs at WARNING, always 204). Both endpoints added to `public_endpoints`. `tests/test_backup_monitoring.py` — **12/12 pass**. Statuses updated in `SECURITY_TODO.md`.
- [x] **Work Diary — per-FSO inspection diary + official report PDF (completed 2026-08-26).** New `app/workdiary/` blueprint (`/workdiary`): read-only diary accumulated from Inspections (Date / Place of Visit / Purpose / Activity), filterable by FSO / date-range / purpose; `/workdiary/preview` + `/workdiary/pdf` render `report.html`, a server-filled reproduction of the official `FSO_Work_Diary_Template.html` (Name/Month/Year meta, roman-numeral column headers (i)–(iv), min-15-row padding, FSO signature + DO countersign), PDF via central `generate_pdf_from_html()` → `send_file(BytesIO)` with sanitized filename and graceful 503 on WeasyPrint failure. Purpose is explicit: new `Inspection.visit_purpose` column (`"routine"|"complaint"|NULL`, migration `add_inspection_visit_purpose`, down_revision `add_fssai_lookup_tables`), required select on the inspection entry form, validated + persisted in create/update routes; `WorkDiaryEngine.derive_purpose` prefers the explicit pick over the legacy problem-presence heuristic (NULL rows keep working). Nav tab added in `base.html`. Lookup reachability regression fixed: `test_route_collisions.py` now seeds its own `FssaiLicense` row instead of depending on bulk-loaded reference data. Files: `app/workdiary/*`, `app/models/inspection.py`, `app/inspection/routes/inspection_routes.py`, `app/inspection/templates/inspection/index.html`, `app/__init__.py`, `migrations/versions/add_inspection_visit_purpose.py`. Research: `docs/WORKDIARY_RESEARCH.md`. `tests/test_workdiary.py` — **28/28 pass** (+2 route-collision green; ruff clean).
...
- [x] **Phase 11 — AI Assistant (completed 2026-08-08).** `app/ai_assistant/` package: `AIAssistantService` (httpx-based, zero new deps), 5 actions (summarize, refine legal language, detect contradictions, suggest annexures, draft prayers), editor sidebar with 5 action buttons, `POST /ai-assistant/assist` with 200/400/503 paths, Celery `run_ai_action` task (lazy import), `app/static/js/ai_assistant.js` IIFE module. Registered in `create_app()` at `url_prefix="/ai-assistant"`. `tests/test_ai_assistant.py` — **23/23 pass**, no regressions.
- [x] **Phase 14 — Knowledge Graph Engine (completed 2026-08-08).** `app/knowledge_graph/`: `KnowledgeGraphEngine` extracts 8 node types + 6 directed edge types from CaseFile/Adjudication; Cytoscape.js view/API routes; SQL persistence to Entity/Relationship tables (case_file only, idempotent); Neo4j Aura sync adapter (`app/knowledge_graph/neo4j_sync.py`) with APOC dynamic labels, 9 uniqueness constraints, 3 property indexes, QStash async + sync fallback route. `tests/test_knowledge_graph.py` — **21/21 pass**; `tests/test_neo4j_kg_sync.py` — **15/15 pass**.
- [x] **Phase 20 — Plugin Architecture (completed 2026-08-18).** `app/plugins/` package: `base.py` (4 provider ABCs + `OCRResult`/`AIResponse` dataclasses), `registry.py` (`PluginRegistry` singleton), `ocr_plugins.py`/`ai_plugins.py`/`rule_plugins.py`/`pdf_plugins.py` concrete providers. All 6 callers refactored to `PluginRegistry.get_active()`. Config-driven selection (`OCR_PROVIDER`/`AI_PROVIDER`/`RULES_PROVIDER`/`PDF_PROVIDER`). Backward-compatible shims preserved. No new dependencies. `tests/test_plugins.py` — **23/23 pass**; `ruff check` + `ruff format --check` clean.
- [x] **RAG Phase 1–5 (completed 2026-08-09).** Full retrieval-augmented generation stack: `app/rag/` with `QdrantStore`, `EmbeddingService`, `Chunker`, `QdrantIndexer`, `IngestionPipeline`, `ContentHasher`/`ChunkDeduper`, `MetadataAdapter`, `CitationAdapter`, `CrossRefAdapter`, `ChunkQualityValidator`; retrieval (`DenseRetriever`, `SparseRetriever`, `HybridRetriever` [RRF k=60], `Reranker`, `QueryClassifier`); generation (`ContextBuilder`, `PromptTemplate`, `GroundedLLMClient`, `CitationTracker`, `ResponseSanitizer`, `GroundedGenerationService`); verification (`ClaimExtractor`, `EvidenceVerifier`, `CitationValidator`, `GroundednessScorer`, `HallucinationDetector`, `TokenCounter`); evaluation (`FaithfulnessMetric`, `AnswerRelevanceMetric`, `ContextPrecisionMetric`, `ContextRecallMetric`, `CitationRecallMetric`, `GroundednessMetric`, `EvalRunner`, `EvalStorage`); `RetrievalLogger`/`RetrievalAuditLog`; resilient pipeline with circuit breaker; LangGraph self-correcting agent pipeline with M5 checkpointing + human-in-the-loop. 694 RAG tests pass (1,780 total — `pytest --collect-only`, 2026-08-20).
...
- [x] **RAG UI Fixes (2026-08-23).** Three gaps from the primary-source audit (`docs/RAG_UI_RESEARCH.md`) closed: **(1) dead "Use agent pipeline" checkbox** — `rag_query.js::getPayload()` now sends `use_agent`, and `/api/rag/query/agent` honors a per-request boolean override over `RAG_USE_AGENT_PIPELINE` (400 on non-bool); **(2) circuit-breaker asymmetry** — Flask `/api/rag/query` now routes through a module-level `ResilientRAGPipeline` singleton (late-bound pipeline fn so tests keep monkeypatching), giving the flagship UI endpoint the same closed→open→half-open protection + stub fallback as `/api/v2/rag/generate`; agent delegation inherits it; **(3) Phase 3 detector dark on the hot path** — `run_generation_pipeline` runs the claim-level `HallucinationDetector` over every generated answer behind new flag `RAG_HALLUCINATION_DETECTOR` (default on, opt-out), escalating sanitizer-missed hallucinated claims (never de-escalating) and attaching a `verification` block to responses; failures degrade best-effort; the UI renders claim-verification stats in `rag_query.js` (new `.rag-verification` block + CSS). Tests: `tests/test_rag_query_breaker.py` (5) + `tests/test_rag_detector_hotpath.py` (4) + `use_agent` overrides in `tests/test_rag_agent_routes.py` (3) — 19/19 pass. Also fixed the stale `TestOCRProvider::test_lazy_import` assertion (imports moved from `extract_text` into `_extract_document`/`_extract_image` during the image-dispatch refactor); `tests/test_plugins.py` **25/25 pass**.
- [x] **RAG UI Gaps #2/#5/#6 (2026-08-23, second batch).** **(2) Stub-mode visibility:** `GroundedLLMClient.use_stub` public property; `/api/rag/health` now reports `llm.mode` ("stub"|"live") + model so ops can assert live LLM in prod; UI shows an amber "Stub mode" banner on `stub-*` answers. **(5) HITL durability signal:** `graph.checkpointer_is_durable()`; `/api/rag/health` exposes `agent_hitl_durable`; both HITL 202 payloads carry `durable`; once-per-process warning when HITL runs on the memory checkpointer (prod still requires `RAG_AGENT_CHECKPOINTER=postgres`). **(6) Session semantics:** localStorage-backed session history in `rag_query.js` (20-turn cap, click-to-reload + question prefill, Clear button). Tests: `tests/test_rag_ui_gaps.py` — **11/11 pass**.
- [x] **RAG UI Gaps #7/#8 (2026-08-23, third batch).** **(7) Live-key A/B flip gate:** `scripts/ab_agent_vs_legacy.py` now defaults to live-LLM mode — aborts when the LLM resolves to stub unless `--allow-stub` (explicitly "NOT flip-gate evidence"); new `check_live_llm` / `summarize` / `parity_verdict` (tolerance 0.05 on gold-hit@10 + groundedness) drive the exit code, so exit 0 is the recorded evidence for flipping `RAG_USE_AGENT_PIPELINE`; summary persisted to `reports/ab_agent_vs_legacy_summary.json`. **(8) CSRF contract pinned:** `tests/test_rag_csrf.py` — rendered page keeps the `csrf-token` meta + `X-CSRFToken` fetch interceptor, token-less/bogus-token POSTs → 400 under `WTF_CSRF_ENABLED=true`, session-token POST passes end-to-end. Tests: `tests/test_ab_parity_gate.py` (13) + `tests/test_rag_csrf.py` (5) — **18/18 pass**.
- [x] **CI/CD gates G1–G14 (completed 2026-08-23).** All 14 CI/CD gaps from `docs/CI_CD_RESEARCH.md` are now implemented and verified by `tests/test_cicd_gates.py` (**46/46 pass**). G1: production deploys gated on green "Repository Validation" via `workflow_run` + `autoDeploy: false`; G2: staging environment (`deploy_staging` job + Render staging service on `upgradation` branch with separate staging DB); G3: `preDeployCommand: flask db upgrade`; G4: `healthCheckPath: /health`; G5: Bandit (HIGH/HIGH) + Safety + pip-audit all **blocking** in `validation.yml` (removed `continue-on-error`/`|| true`); G6: `fail_under = 60`; G7: Dockerfile `ENTRYPOINT` + `CMD → uvicorn asgi:app` (ASGI); G8: `release.yml` activated (`push: tags` + `workflow_dispatch` → `softprops/action-gh-release@v2`); G9: Dependabot (pip + actions + npm, `rebase-strategy: all`); G10: checkout@v7 + setup-python@v7 + ruff≥0.16.3 + ubuntu-24.04 + concurrency groups; G11: ce-v2 `real-gate` dispatch-only; G12: `shared-secrets` envVarGroup with single `SECRET_KEY`; G13: deploy serialization via `concurrency` group; G14: pip-audit scans `requirements-dev.txt`. **Setup required:** create Render deploy hooks (`RENDER_DEPLOY_HOOK_URL`, `RENDER_STAGING_DEPLOY_HOOK_URL`) + GitHub `staging` environment.

- [x] **Phase 18 RBAC — Complete (2026-08-26).** Wired the pre-existing `Role`/`user_roles` tables: roles `admin` (bypass) + `fso`; central gate in `app/__init__.py::enforce_rbac` driven by `ROLE_BLUEPRINTS` in `app/shared/rbac.py`; record-level scoping via `scoped_officer_name()` + `case_visible_to_user()` across CaseFile, Adjudication, Inspection, Work Diary, Comments API; create-routes force-stamp the bound officer server-side; users bound via `users.fso_name` (migration `add_user_fso_name`). Provisioning seam `app/auth/provisioning.py` + extended admin Add User form + bulk seed `scripts/seed_fso_users.py`; existing users backfilled to `admin`. Notepad blueprint registered; Comments API at `app/comments/`. `tests/test_rbac.py` — **44/44 pass**.
- [x] **Work Diary — Complete (2026-08-26).** `app/workdiary/` blueprint: per-FSO inspection diary (Date / Place of Visit / Purpose / Activity); purpose from explicit `Inspection.visit_purpose` pick with legacy heuristic fallback (migration `add_inspection_visit_purpose`); filterable by FSO/date-range/purpose; `/workdiary/preview` + `/workdiary/pdf` render official `FSO_Work_Diary_Template.html` via `generate_pdf_from_html()`. `tests/test_workdiary.py` — **28/28 pass**. Research: `docs/WORKDIARY_RESEARCH.md`.
- [x] **Security close-out — S10c + S2 (2026-08-26).** `run_backup()` bookkeeping to `settings` table; public `GET /health/backups` dead-man's-switch (200/503 for never/stale/stale>26h/degraded); `POST /csp-report` collector (CSRF-exempt, both report formats, bounded 4 KB body). `tests/test_backup_monitoring.py` — **12/12 pass**. Statuses updated in `SECURITY_TODO.md`.
- [x] **Redis/Celery ssl_cert_reqs fix + Sync fallback (2026-08-26).** `_normalize_redis_url()` in `celery_app.py` appends `?ssl_cert_reqs=2` to `rediss://` URLs (used for both `broker_url` and `result_backend`); `_run_task_inline()` in `app/utils/qstash_client.py` calls `task.run(**payload)` for Celery Tasks, falls back to `.apply()` for non-Celery shims; sync fallback in `publish_task` + task-webhook route (`app/tasks_webhook/routes.py`) use it. Fixes the `ValueError: A rediss:// URL must have parameter ssl_cert_reqs…` crash from case_file_generator's async PDF task dispatch (Celery's `RedisBackend.__init__`). Verified: before fix returned `500 {"error": "Case file PDF generation failed: ValueError: A rediss:// URL must have parameter ssl_cert_reqs…"}`; after fix returns clean `500 {"error": "PDF assembly failed: Failed to generate main document PDFs"}` (the real WeasyPrint-missing error). `tests/test_sync_fallback_fix.py` — **11/11 pass**. Commit `e41a396`.
- [x] **Case File Document Preview (TDD, 2026-08-26).** `POST /case_file_generator/preview` renders Petition + Permission Letter HTML from form data without creating a CaseFile record or dispatching a PDF task. Quill editor modal with dual iframe tabs in `index.html`. RBAC officer stamping applied. `tests/test_preview_case_file.py` — **9/9 pass**. Commit `96e4349`.
- [x] **Adjudication Document Preview (TDD, 2026-08-26).** `POST /adjudication/preview` mirrors the case file preview: renders petition + permission letter HTML for Quill editor review before committing to `/generate_all`. Key finding: adjudication `/generate_all` uses **synchronous in-memory** PDF generation (no Celery/Redis), so it was NOT affected by the Redis ssl_cert_reqs crash — only lacked the preview-before-generate UX. Added `validate_adjudication_form()` + `preview_adjudication_route()` + Preview button/modal/JS in template. `tests/test_preview_adjudication.py` — **9/9 pass**. Commit `93950a3`.

### 📌 Suggested Next 3 Steps (updated 2026-08-23)

> Highest future impact, smallest effort — in this order. **Phases 11–16, 20, 21, A + OCR B–E, Priority 6, Priority 7, Deepening D1–D5, S9a, S6a–d, Phase 15, Phase 18, Phase 21, S10c, S2, CI/CD gates G1–G14, Case File Preview, Adjudication Preview, Redis/Celery fix — all ✅ DONE** (verified through 2026-08-26). RAG Phase 1–5 ✅, Multi-Domain Phase 1 ✅, FSSAI re-ingest ✅, Evaluation ✅, Benchmark ✅, Rust ✅, Remote Inference ✅, LangGraph ✅, M5 ✅, FastAPI Gateway ✅, Config seam ✅. **Phase 6 deferred**; Phase 17 partial (Supabase bridge / conflict resolution / sync-status UI remain). Re-rolled list:

1. **Phase 17 — Cloud Sync / Supabase Bridge** (largest remaining gap). R2/B2 + Cloudinary + Sheets/Airtable/Excel sync done (Priority 7); only the Supabase bridge, conflict resolution, and sync-status UI remain. New `app/sync/` blueprint per plan.md §3.2.
2. **Phase 19 — AI Case Intelligence** (`app/case_intelligence/`). Synthesize Legal Validation Engine (Phase 12) outputs + AI LLM (Phase 11) to produce a composite Case Readiness Score (0–100), evidence strength index, and allegation-to-evidence matrix. Deliverable: `GET /case_intelligence/<id>` + `tests/test_case_intelligence.py`. Natural consumer of the corpus KG (`kg/`) and Phase 14 case KG.
3. **RAG Query Interface UI** (task.md §7 — backend done, frontend form/template/JS/nav-link + 8 tests still planned).
4. **Rust Parts 2–5** — port fuzzy helpers / TOC / cross-reference to `nsa_rust` (Part 1 source complete; build blocked on Windows, works on Linux).
5. **ENV-10** — confirm Render dashboard env vars (Modal deploy + secrets done 2026-08-16; dashboard env vars pending).
...
### Phase 15: Analytics Dashboard — ✅ DONE (completed 2026-08-22)
...
- **Status (completed 2026-08-22):** ✅ `app/analytics/` blueprint implemented at `/analytics`: interactive dashboard (Chart.js charts + Leaflet FBO map) fed by `GET /analytics/api/metrics` (summary counts across six major tables, monthly case trends, inspection compliance, sample pipeline by billed state, legal provisions cited, FSO activity, FBO issue states, evidence types, geo coordinates); lightweight aggregate SQL; nav link in `base.html`. `tests/test_analytics.py` — **15/15 pass**. Note: monthly grouping uses SQLite `strftime` — swap for a portable expression when PostgreSQL analytics matter.
- **Target Files to Edit/Create:**
...
### Phase 18: Multi-User RBAC & Document Comments — ✅ COMPLETE (completed 2026-08-26)
...
- **D7 (audit)** — ✅ DONE (2026-09-09). The factory turned out to be the right seam after all: `app/services/audit_context.py` binds `entity_type`, normalizes `actor`, and swallows writer failures; the call-time-resolved `_default_writer` gives tests one patch point for all instances. All step-4 direct callers migrated the same day; only the RAG wrapper modules and the `app/inspection/audit.py` re-export shim still reference the core directly (by design).
...
## AUTH-1: /api/v2 vs Flask auth divergence (recorded 2026-08-22, deferred)

The same operations are protected differently per transport: Flask routes sit
behind session auth + CSRF (`require_login` global gate), while every
`/api/v2/*` route is guarded only by `ApiKeyAuthMiddleware` — a static API key
that is OPTIONAL (when `API_V2_KEY` is unset, all of /api/v2 is public,
including `/api/v2/docs`). Notably `/api/v2/ai-assistant/assist`,
`/api/v2/search/reindex`, and `/api/v2/rag/ingest*` mutate state or spend LLM
tokens yet are effectively unauthenticated in dev-mode ASGI deployments.

Decision (2026-08-22 architecture review): out of scope for the transport-
