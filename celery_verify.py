...
missing_tables = [t for t in ['bills', 'case_files', 'sample', 'fso', 'inspection']
...
for table, col in [('case_files', 'pdf_task_id'), ('bills', 'pdf_task_id'),
                   ('case_files', 'pdf_generated_at'), ('bills', 'pdf_generated_at')]:
...
from app.extensions import db
from app.models import Bill, CaseFile, Sample, FSO, BillSample
...
try:
    ...
    if not _TESS_OK:
        record("run_ocr_extraction", "enqueue+execution", SKIP_MARK,
        ...
        record("run_ocr_extraction", "enqueue+execution", OK,
               f"task_id={task.id} | pages={r['_pages_processed']}")
except Exception as e:
    record("run_ocr_extraction", "enqueue+execution", FAIL_MARK, str(e)[:200])

# --- Bill PDF ---
try:
    bill = Bill(Name="E2E BILL", EMP_ID="FSO-E2E",
                Enf_samp_No=2, Surv_samp_No=1,
                enforcement_price=3000, surveillance_price=2000,
                Total_bill=5000, No_of_enfbills=2, No_of_survbills=1,
                TR_Value="TRE2E",
    ...
    tv = {"Name": "E2E BILL", "EMP_ID": "FSO-E2E", "Designation": "FSO",
          "Enf_samp_No": "2", "Surv_samp_No": "1", "Total_bill": "5000",
          "No_of_enfbills": "2", "No_of_survbills": "1", "TR_Value": "TRE2E",
          "TR_date": "15/01/2026", "Submission_date": "20/01/2026",
    ...
    if not _WP_OK:
        record("generate_bill_pdf", "enqueue+execution", SKIP_MARK,
        ...
        record("generate_bill_pdf", "enqueue+execution", s,
               f"task_id={task.id} | status={r['status']}")
...
# --- CaseFile PDF ---
try:
    cf = CaseFile(case_number="E2E/CF/001",
                  food_safety_officer_name="Dr. E2E",
                  authorization_date=datetime(2026, 1, 15),
                  inspection_date=datetime(2026, 1, 10),
                  inspection_time="10:00",
                  manufacturer_fssai="MFGE2E", manufacturer_name="John",
                  manufacturer_fbo_name="E2E Mfg", manufacturer_address="123",
                  retailer_fssai="RETE2E", retailer_name="Jane",
                  retailer_fbo_name="E2E Retail", retailer_address="456",
                  product_name="E2E Product", batch_no="B001",
                  sample_quantity="500g", packet_count=4,
                  mfg_date=datetime(2026, 1, 1), expiry_date=datetime(2026, 12, 31),
                  sample_code="SMPE2E", sample_submission_date=datetime(2026, 1, 12),
                  Lab_Registration_No="LABE2E", do_receipt_date=datetime(2026, 1, 14),
                  is_misbranded=False, is_substandard=True,
                  analyst_report_no="ARE2E", analyst_report_date=datetime(2026, 1, 20),
                  directive_letter_no="DLE2E", directive_letter_date=datetime(2026, 1, 22),
                  retailer_report_receive_date=datetime(2026, 1, 25),
                  manufacturer_report_receive_date=datetime(2026, 1, 26),
                  applicable_regulation="2.4.1", applicable_clause="2.4.1(1)",
                  sample_name="E2E Sample", applicable_sections="51")
    db.session.add(cf)
    db.session.commit()
    cid = cf.id

    cd = {"case_number": "E2E/CF/001", "food_safety_officer_name": "Dr. E2E",
          "authorization_date": "15/01/2026", "inspection_date": "10/01/2026",
          "inspection_time": "10:00",
          "manufacturer_fssai_license": "MFGE2E", "manufacturer_person_name": "John",
          "manufacturer_trade_name": "E2E Mfg", "manufacturer_address": "123",
          "retailer_fssai_license": "RETE2E", "retailer_person_name": "Jane",
          "retailer_trade_name": "E2E Retail", "retailer_address": "456",
          "product_name": "E2E Product", "batch_no": "B001",
          "sample_quantity": "500g", "packet_count": 4,
          "mfg_date": "01/01/2026", "expiry_date": "31/12/2026",
          "sample_code": "SMPE2E", "sample_submission_date": "12/01/2026",
          "lab_registration_no": "LABE2E", "do_receipt_date": "14/01/2026",
          "is_misbranded": False, "is_substandard": True,
          "analysis_result": "substandard", "applicable_sections": ["51"],
          "applicable_sections_str": "51", "applicable_regulation": "2.4.1",
          "applicable_clause": "2.4.1(1)", "sample_name": "E2E Sample",
          "total_cost": "500", "cost_in_words": "Five Hundred Only",
          "analyst_report_no": "ARE2E", "analyst_report_date": "20/01/2026",
          "directive_letter_no": "DLE2E", "directive_letter_date": "22/01/2026",
    ...
    if not _WP_OK:
        record("generate_case_file_pdf", "enqueue+execution", SKIP_MARK,
               f"task_id={task.id} | WeasyPrint not available (enqueue OK)")
        ...
        r = task.get(timeout=60)
        s = OK if r["status"] == "ok" else FAIL_MARK
        record("generate_case_file_pdf", "enqueue+execution", s,
               f"task_id={task.id} | status={r['status']}")
        if r.get("file_path") and os.path.exists(r["file_path"]):
            os.remove(r["file_path"])
        db.session.delete(cf)
...
with app.test_client() as client:
    # --- Bill route ---
    try:
        s1 = Sample(sample_code="VRFY001", sample_name="Verify 1",
                    sample_type="enforcement", fso_name="VERIFY_FSO",
                    collection_date=datetime(2026, 1, 15), price="1500", billed=False)
        s2 = Sample(sample_code="VRFY002", sample_name="Verify 2",
                    sample_type="surveillance", fso_name="VERIFY_FSO",
                    collection_date=datetime(2026, 1, 20), price="1000", billed=False)
        db.session.add(s1)
        db.session.add(s2)
        db.session.commit()

        resp = client.post("/bill_generator/generate_bill", data={
            "start_date": "2026-01-01", "end_date": "2026-01-31",
            "Name": "Route Test FBO", "EMP_ID": "FSO-RT",
            "Designation": "FSO", "TR_Value": "TRRT001",
            "TR_date": "2026-01-25", "Submission_date": "2026-01-30",
            "No_of_enfbills": "1", "No_of_survbills": "1",
        })
        ...
        record("generate_bill_pdf", "route_async_behavior", OK,
               f"HTTP 202 | task_id={body['task_id']} | bill_id={body['bill_id']} | "
    ...
    # --- CaseFile route ---
    try:
        resp = client.post("/case_file_generator/generate_case_file", data={
            ...
            "product_name": "Route Product", "batch_no": "BATCHRT",
            "sample_quantity": "250g", "packet_count": "2",
            "mfg_date": "2026-01-01", "expiry_date": "2026-12-31",
            "sample_code": "SMPRT", "sample_submission_date": "2026-02-12",
            "lab_registration_no": "LABRT", "do_receipt_date": "2026-02-14",
            "is_substandard": "substandard",
            "analyst_report_no": "ARRT", "analyst_report_date": "2026-02-20",
            "directive_letter_no": "DLRT", "directive_letter_date": "2026-02-22",
        ...
        record("generate_case_file_pdf", "route_async_behavior", OK,
    ...
    try:
        ...
        if cf:
            ...
            record("generate_case_file_pdf", "regenerate_async_behavior", OK,
...
record("run_ocr_extraction", "worker_execution", SKIP_MARK,
       "Requires real worker -- see instructions above")
record("generate_bill_pdf", "worker_execution", SKIP_MARK,
       "Requires real worker -- see instructions above")
record("generate_case_file_pdf", "worker_execution", SKIP_MARK,
       "Requires real worker -- see instructions above")
...
try:
    cnt = FSO.query.count()
    record("run_ocr_extraction", "app_context", OK,
...
try:
    html = render_template("bill_generator/template.html",
                           Name="T", EMP_ID="T", Designation="T",
                           Enf_samp_No="0", Surv_samp_No="0",
                           Total_bill="0", No_of_enfbills="0", No_of_survbills="0",
                           TR_Value="", TR_date="", Submission_date="",
                           start_date="", end_date="", enforcement_price="0", surveillance_price="0")
...
try:
    html = render_template("case_file_generator/petition.html",
                           case_number="T/001", food_safety_officer_name="Dr.T",
                           authorization_date="15/01/2026", inspection_date="10/01/2026",
                           inspection_time="10:00",
                           manufacturer_fssai_license="MFG", manufacturer_person_name="John",
                           manufacturer_trade_name="Co", manufacturer_address="Addr",
                           retailer_fssai_license="RET", retailer_person_name="Jane",
                           retailer_trade_name="Ret", retailer_address="Addr",
                           product_name="P", batch_no="B", sample_quantity="Q", packet_count=1,
                           mfg_date="01/01/2026", expiry_date="31/12/2026",
                           sample_code="S", sample_submission_date="12/01/2026",
                           lab_registration_no="L", do_receipt_date="14/01/2026",
                           is_misbranded=False, is_substandard=True,
                           analysis_result="substandard", applicable_sections=["51"],
                           applicable_sections_str="51", applicable_regulation="2.4.1",
                           applicable_clause="2.4.1(1)", sample_name="S",
                           total_cost="500", cost_in_words="Five Hundred",
                           analyst_report_no="AR", analyst_report_date="20/01/2026",
                           directive_letter_no="DL", directive_letter_date="22/01/2026",
                           retailer_report_receive_date="25/01/2026",
                           manufacturer_report_receive_date="26/01/2026")
...
if not _TESS_OK:
    record("run_ocr_extraction", "result_correctness", SKIP_MARK,
    ...
    try:
        ...
        assert isinstance(r, dict) and "_pages_processed" in r
        record("run_ocr_extraction", "result_correctness", OK,
...
record("run_ocr_extraction", "result_vs_old_version", SKIP_MARK,
...
if not _WP_OK:
    ...
    try:
        bill = Bill(Name="RESULT CHK", EMP_ID="FSO-RC",
                    Enf_samp_No=1, Surv_samp_No=0,
                    enforcement_price=1000, surveillance_price=0,
                    Total_bill=1000, No_of_enfbills=1, No_of_survbills=0,
                    TR_Value="TRRC", TR_date=datetime(2026, 3, 15),
                    Submission_date=datetime(2026, 3, 20),
                    start_date=datetime(2026, 3, 1), end_date=datetime(2026, 3, 31))
        db.session.add(bill)
        db.session.commit()
        bid = bill.id

        tv = {"Name": "RESULT CHK", "EMP_ID": "FSO-RC", "Designation": "FSO",
              "Enf_samp_No": "1", "Surv_samp_No": "0", "Total_bill": "1000",
              "No_of_enfbills": "1", "No_of_survbills": "0", "TR_Value": "TRRC",
              "TR_date": "15/03/2026", "Submission_date": "20/03/2026",
        ...
        record("generate_bill_pdf", "result_correctness", OK,
    ...
    try:
        cf = CaseFile(case_number="RC/CF/001",
                      ...
                      manufacturer_fssai="MFGRC", manufacturer_name="John RC",
                      manufacturer_fbo_name="RC Mfg", manufacturer_address="RC",
                      retailer_fssai="RETRC", retailer_name="Jane RC",
                      retailer_fbo_name="RC Retail", retailer_address="RC",
                      product_name="RC Product", batch_no="BATCHRC",
                      sample_quantity="100g", packet_count=1,
                      mfg_date=datetime(2026, 1, 1), expiry_date=datetime(2026, 12, 31),
                      sample_code="SMPRC", sample_submission_date=datetime(2026, 3, 12),
                      Lab_Registration_No="LABRC", do_receipt_date=datetime(2026, 3, 14),
                      is_misbranded=False, is_substandard=True,
                      analyst_report_no="ARRC", analyst_report_date=datetime(2026, 3, 20),
                      directive_letter_no="DLRC", directive_letter_date=datetime(2026, 3, 22),
                      retailer_report_receive_date=datetime(2026, 3, 25),
                      manufacturer_report_receive_date=datetime(2026, 3, 26),
                      applicable_regulation="2.4.1", applicable_clause="2.4.1(1)",
                      sample_name="RC Sample", applicable_sections="51")
        db.session.add(cf)
        db.session.commit()
        cid = cf.id

        cd = {"case_number": "RC/CF/001", "food_safety_officer_name": "Dr. RC",
              "authorization_date": "15/03/2026", "inspection_date": "10/03/2026",
              "inspection_time": "09:30",
              "manufacturer_fssai_license": "MFGRC", "manufacturer_person_name": "John RC",
              "manufacturer_trade_name": "RC Mfg", "manufacturer_address": "RC",
              "retailer_fssai_license": "RETRC", "retailer_person_name": "Jane RC",
              "retailer_trade_name": "RC Retail", "retailer_address": "RC",
              "product_name": "RC Product", "batch_no": "BATCHRC",
              "sample_quantity": "100g", "packet_count": 1,
              "mfg_date": "01/01/2026", "expiry_date": "31/12/2026",
              "sample_code": "SMPRC", "sample_submission_date": "12/03/2026",
              "lab_registration_no": "LABRC", "do_receipt_date": "14/03/2026",
              "is_misbranded": False, "is_substandard": True,
              "analysis_result": "substandard", "applicable_sections": ["51"],
              "applicable_sections_str": "51", "applicable_regulation": "2.4.1",
              "applicable_clause": "2.4.1(1)", "sample_name": "RC Sample",
              "total_cost": "500", "cost_in_words": "Five Hundred Only",
              "analyst_report_no": "ARRC", "analyst_report_date": "20/03/2026",
              "directive_letter_no": "DLRC", "directive_letter_date": "22/03/2026",
        ...
        record("generate_case_file_pdf", "result_correctness", OK,
               f"Meta-only | ZIP valid | contents={names}")
        os.remove(r["file_path"])
        db.session.delete(cf)
        db.session.commit()
    except Exception as e:
...
try:
    try:
        ...
        record("run_ocr_extraction", "retry_non_transient_format", FAIL_MARK,
               "Expected ValueError for unsupported format")
    except ValueError:
        record("run_ocr_extraction", "retry_non_transient_format", OK,
               "Invalid format -> ValueError (no retry)")
    except Exception as e:
        record("run_ocr_extraction", "retry_non_transient_format", FAIL_MARK,
...
try:
    try:
        ...
        record("run_ocr_extraction", "retry_non_transient_missing", FAIL_MARK,
               "Expected ValueError for missing file")
    except ValueError:
        record("run_ocr_extraction", "retry_non_transient_missing", OK,
               "Missing file -> ValueError (no retry)")
    except Exception as e:
        record("run_ocr_extraction", "retry_non_transient_missing", FAIL_MARK,
...
record("run_ocr_extraction", "retry_transient_io", SKIP_MARK,
...
try:
    ...
    if r["status"] == "error":
        record("generate_bill_pdf", "retry_template_error", OK,
...
if not _WP_OK:
    ...
    try:
        bill = Bill(Name="IDEM BILL", EMP_ID="FSO-ID",
                    Enf_samp_No=1, Surv_samp_No=0,
                    enforcement_price=500, surveillance_price=0,
                    Total_bill=500, No_of_enfbills=1, No_of_survbills=0,
                    TR_Value="TRID", TR_date=datetime(2026, 4, 15),
                    Submission_date=datetime(2026, 4, 20),
                    start_date=datetime(2026, 4, 1), end_date=datetime(2026, 4, 30))
        db.session.add(bill)
        db.session.commit()
        bid = bill.id
        tv = {"Name": "IDEM BILL", "EMP_ID": "FSO-ID", "Designation": "FSO",
              "Enf_samp_No": "1", "Surv_samp_No": "0", "Total_bill": "500",
              "No_of_enfbills": "1", "No_of_survbills": "0", "TR_Value": "TRID",
              "TR_date": "15/04/2026", "Submission_date": "20/04/2026",
        ...
        record("generate_bill_pdf", "idempotency", OK,
               f"2 runs, same output path | 1 bill record | no duplicates")
    ...
    try:
        cf = CaseFile(case_number="IDEM/CF/002",
                      ...
                      manufacturer_fssai="MFGID", manufacturer_name="John ID",
                      manufacturer_fbo_name="ID Mfg", manufacturer_address="ID",
                      retailer_fssai="RETID", retailer_name="Jane ID",
                      retailer_fbo_name="ID Retail", retailer_address="ID",
                      product_name="ID Product", batch_no="BATCHID",
                      sample_quantity="100g", packet_count=1,
                      mfg_date=datetime(2026, 1, 1), expiry_date=datetime(2026, 12, 31),
                      sample_code="SMPID", sample_submission_date=datetime(2026, 4, 12),
                      Lab_Registration_No="LABID", do_receipt_date=datetime(2026, 4, 14),
                      is_misbranded=False, is_substandard=True,
                      analyst_report_no="ARID", analyst_report_date=datetime(2026, 4, 20),
                      directive_letter_no="DLID", directive_letter_date=datetime(2026, 4, 22),
                      retailer_report_receive_date=datetime(2026, 4, 25),
                      manufacturer_report_receive_date=datetime(2026, 4, 26),
                      applicable_regulation="2.4.1", applicable_clause="2.4.1(1)",
                      sample_name="ID Sample", applicable_sections="51")
        db.session.add(cf)
        db.session.commit()
        cid = cf.id

        cd = {"case_number": "IDEM/CF/002", "food_safety_officer_name": "Dr. Idem",
              "authorization_date": "15/04/2026", "inspection_date": "10/04/2026",
              "inspection_time": "09:00",
              "manufacturer_fssai_license": "MFGID", "manufacturer_person_name": "John ID",
              "manufacturer_trade_name": "ID Mfg", "manufacturer_address": "ID",
              "retailer_fssai_license": "RETID", "retailer_person_name": "Jane ID",
              "retailer_trade_name": "ID Retail", "retailer_address": "ID",
              "product_name": "ID Product", "batch_no": "BATCHID",
              "sample_quantity": "100g", "packet_count": 1,
              "mfg_date": "01/01/2026", "expiry_date": "31/12/2026",
              "sample_code": "SMPID", "sample_submission_date": "12/04/2026",
              "lab_registration_no": "LABID", "do_receipt_date": "14/04/2026",
              "is_misbranded": False, "is_substandard": True,
              "analysis_result": "substandard", "applicable_sections": ["51"],
              "applicable_sections_str": "51", "applicable_regulation": "2.4.1",
              "applicable_clause": "2.4.1(1)", "sample_name": "ID Sample",
              "total_cost": "100", "cost_in_words": "One Hundred",
              "analyst_report_no": "ARID", "analyst_report_date": "20/04/2026",
              "directive_letter_no": "DLID", "directive_letter_date": "22/04/2026",
        ...
        record("generate_case_file_pdf", "idempotency", OK,
               f"2 runs, same output path | no duplicate records")
        os.remove(r1["file_path"])
        db.session.delete(cf)
        db.session.commit()
    except Exception as e:
