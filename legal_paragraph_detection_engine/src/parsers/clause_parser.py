...
class ClauseData:
    ...
    children: list["ClauseData"] = field(default_factory=list)
...
class ClauseParser:
    ...
    CLAUSE_PATTERNS: ClassVar[list[ClausePattern]] = [
        ...
        ClausePattern(
            ...
            regex=r"^\s*\d+\s*\(\s*\d+\s*\)\s*\(\s*[a-zA-Z]\s*\)\s*(?:\([i-ivIVX]{1,4}\))?\s*(.*?)\s*$",
            priority=30,
        ...
        ClausePattern(
            ...
            regex=r"^\s*(?:Explanation|Illustration|Example)\s*(?:\d*)?\s*:?\s*(.*?)\s*$",
            priority=40,
            description="Explanation sections",
        ),
        ClausePattern(
            pattern_type="proviso",
            regex=r"^\s*(?:PROVISO|Provided|Provided further)\s*:?\s*(.*?)\s*$",
            priority=40,
            description="Proviso sections",
        ),
        ClausePattern(
            pattern_type="exception",
            regex=r"^\s*Except\s*(?:that)?\s*:?\s*(.*?)\s*$",
            priority=40,
            description="Exception clauses",
        ),
        ClausePattern(
            pattern_type="note",
            regex=r"^\s*(?:Note|Notes|IMPORTANT)\s*:?\s*(.*?)\s*$",
            priority=40,
            description="Notes and important notices",
        ),
        # Reference patterns
        ClausePattern(
        ...
        ClausePattern(
            ...
            regex=r"^\s*(Schedule|Table)\s*([A-Z0-9IVX]*)?\s*:?\s*(.*?)\s*$",
            priority=60,
    ...
    def parse_clauses(self, text: str) -> list[ClauseData]:
        ...
        with self._lock:
            ...
            # Validate ID uniqueness (line-derived ids must not collide)
            ids = [c.id for c in clauses]
            if len(ids) != len(set(ids)):
                raise RuntimeError("ClauseParser produced duplicate clause ids — check id derivation")
    ...
    def _match_clause_pattern(self, line: str, line_num: int) -> ClauseData | None:
        ...
        for pattern in sorted_patterns:
            ...
            if match:
                ...
                # IDs are derived from the line number (T-06, F-10): stable,
                # deterministic, and they survive clear_cache(). Each line can
                # produce at most one clause, so clause_{line_num} is unique
                # within a document.
                return ClauseData(
    ...
    def _build_clause_hierarchy(self, clauses: list[ClauseData]) -> list[ClauseData]:
        ...
        for clause in clauses:
            ...
            if level > 1:
                ...
                # Exclude self and require a true predecessor (parent must precede child).
                candidates = [
                    c
                    for c in clauses
                    if c is not clause
                    and c.start_line < clause.start_line
                    and c.clause_type == clause.clause_type
                    and self._is_parent_of(c.text, clause.text)
                ]
                if candidates:
    ...
    def _is_parent_of(self, parent_text: str, child_text: str) -> bool:
        ...
        if parent_numbers and child_numbers:
            ...
            return bool(parent_numbers[0] == child_numbers[0])
    ...
    def clear_cache(self):
        ...
        IDs are line-derived (T-06), so clearing the cache does not affect the
        ids of future parses — no counter state to reset.
        """
        with self._lock:
            self._cache.clear()
