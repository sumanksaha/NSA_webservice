...
from .cache import evict_if_full, stable_key
...
class TextCleaner:
    ...
    PRESERVE_PATTERNS: ClassVar[dict[str, list[str]]] = {
        "legal_citations": [
    ...
    HIERARCHY_MARKERS: ClassVar[list[str]] = [
    ...
    def __init__(self, continuation_max_words: int = 3):
    ...
    def _classify_line_type(self, line: str) -> TextType:
        ...
        # Check for footers BEFORE headers so lines like
        # "Copyright: All rights reserved." stay footers even though they
        # also match the "Label: value" header pattern below.
        if re.search(r"(?:copyright|page \d+ of|\d+ / \d+ pages)", line_lower):
            return TextType.FOOTER

        # Check for section headers ("Label" or "Label: value")
    ...
    def _clean_non_legal_line(self, line: str) -> str:
        ...
        cleaned = re.sub(r"^\s*(?:Table|Fig)\s*[A-Z]\s*\d*\s*:", "", cleaned, flags=re.IGNORECASE)
    ...
    def _continues_previous_line(self, prev_line: str, current_line: str) -> bool:
        ...
        # or if current line doesn't look like a new hierarchy item. The word
        # count threshold is configurable (T-34, default 3).
