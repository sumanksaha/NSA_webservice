...
- **Stack:** Flask 2.x + SQLAlchemy 2.x + PostgreSQL (primary) / SQLite (dev) + WeasyPrint + Redis + QStash
...
│   │   └── tasks.py              # QStash task for DO intimation PDF + sync
...
│   │   ├── tasks.py              # QStash task for Neo4j sync
...
QStash (webhook-based, no worker required on free tier) is the only async
transport — Celery was removed 2026-09-13 (`celery_app.py` deleted; tasks are
plain functions in `TASK_REGISTRY` with synchronous inline fallback).
...
| `REDIS_URL`                                                              | QStash task-status store + frontend polling                                                                                                                                                                                                                                                                                                                                               |
...
| `ENABLE_SNAPSHOT_SCHEDULE`                                               | Register the nightly QStash local DB snapshot schedule (`create_daily_db_snapshot`, midnight UTC) at startup via `ScheduledJobs` (default true — opt-out; ex-Celery-beat parity)                                                                                                                     |
| `RAG_ENABLE_LOG_CLEANUP_SCHEDULE`                                        | Register the weekly QStash RAG query-log retention cleanup (`cleanup_rag_query_logs`, Sundays 04:00 UTC) at startup via `ScheduledJobs` (default true — opt-out; retention runs unless explicitly disabled)                                                                                          |
...
> **Reconciliation (2026-08-26):** per-file counts below have drifted as phases landed;
> `pytest --collect-only` is authoritative — **~1,970 tests total**, of which **694 are
> RAG-related** (full audit inventory in `RAG_AUDIT_REPORT.md` §2). Phase 1 multi-domain
> adds `tests/test_multidomain_phase1.py` (37). RBAC (44), Work Diary (28), backup
> monitoring (12), Redis fix (11), Case File Preview (9), Adjudication Preview (9) also
> added since the 2026-08-20 collect.
...
| test_preview_case_file.py        | 9     | Case File Preview (TDD): `POST /case_file_generator/preview` — form validation, template render, JSON response (petition + permission), RBAC stamping                                  |
| test_preview_adjudication.py     | 9     | Adjudication Preview (TDD): `POST /adjudication/preview` — form validation, template render, JSON response, RBAC stamping                                          |
| test_sync_fallback_fix.py        | 5     | Sync fallback: `_run_task_inline()` (2), sync-fallback integration (3)                                                                                                     |
