...
**TL;DR:** The RAG intelligence-layer **modules are implemented** (structured query decomposition, adaptive retrieval strategies, three‑stage reranking, evidence optimization, targeted retry with failure diagnosis, answerability gates, separate confidence metrics, KG‑based reasoning). **Phases 0–2 of the V2 plan are complete (2026‑09‑09):** the agent graph's wiring defects are fixed; the EvidenceTask DAG executes as **real parallel waves** (requirements-inferred dependencies, thread‑pool fan‑out via `RAG_AGENT_TASK_PARALLELISM`, deterministic cross‑reference expansion, per‑task results in `agent.task_results`); and verification now runs at **two levels** — a per‑task 7‑signal sufficiency rubric (coverage, relevance, authority, specificity, completeness, contradiction, temporal validity) gates synthesis before generation, while rule‑based **claim‑level entailment** checks every factual claim of the generated answer against the evidence (claim‑groundedness on the response; mostly‑unverified claims trigger targeted retrieval). Live contradiction detection (numeric + prohibition conflicts within the same provision) and authority scoring (document‑type tier × name hierarchy) feed the FailureClassifier taxonomy directly. Agent suite: 149 passed. Phases 3–4 (complexity/budget routing economics, gold‑dataset evaluation) remain. See [`docs/V2_PLANNER_EXECUTOR_VERIFIER_PLAN.md`](V2_PLANNER_EXECUTOR_VERIFIER_PLAN.md).
