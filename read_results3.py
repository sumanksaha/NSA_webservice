...
for key in sorted(pq.keys()):
    if key.startswith("ce_v2_K500_"):
        ...
        print(
            f"    gold_in_pool={d.get('gold_in_pool')}, gold_in_rrf={d.get('gold_in_rrf')}, "
...
gold_in_top10 = sum(
    1 for k, v in pq.items() if k.startswith("ce_v2_K500_") and v.get("details", {}).get("gold_in_ce_top10", False)
)
...
gold_in_top10_v1 = sum(
    1 for k, v in pq.items() if k.startswith("ce_v1_") and v.get("details", {}).get("gold_in_ce_top10", False)
)
