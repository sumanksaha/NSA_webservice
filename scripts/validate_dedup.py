...
try:
    ...
    print("  dedup_group_assignments.csv:         NOT FOUND (no merge performed)")
...
if len(assignments) > 0:
    ...
    if same_group_count > 0:
        ...
        print("  but are in the same dedup group. Review priority.")
else:
...
if len(assignments) > 0:
    ...
    if overlap:
        print("  These fbo_ids were merged but still need review. Flagged.")
else:
    print("  [SKIP] No assignments file — no merge performed")
...
try:
    ...
    print(
        f"  Change: {backup_single - current_single:,} singletons consolidated into "
