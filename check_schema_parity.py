...
migration_case_files_cols = {
    ...
    "inspection_date", "inspection_time", "sample_id", "manufacturer_fssai",
    ...
    "analyst_report_no", "analyst_report_date", "directive_letter_no",
    "directive_letter_date", "retailer_report_receive_date",
    "manufacturer_report_receive_date", "applicable_regulation",
...
migration_sample_cols = {
    ...
    "price", "billed", "created_at", "synced_at",
}
...
migration_inspection_cols = {
    ...
    "inspection_date", "compliance_deadline", "is_dismissed", "dismissed_by",
    "dismissed_at", "adjudication_id", "created_at", "synced_at",
}

# Bills: created by add_missing_base_tables, altered by add_bill_sample_fields, fix_schema_datetime_fk
migration_bills_cols = {
    "id", "Name", "EMP_ID", "Designation", "Enf_samp_No", "Surv_samp_No",
    "enforcement_price", "surveillance_price", "Total_bill",
    "No_of_enfbills", "No_of_survbills", "TR_Value", "TR_date",
    "Submission_date", "start_date", "end_date", "created_at", "synced_at",
}
...
for tname in sorted(live.keys()):
    ...
    print(f"{tname:<25} {f'{len(expected - missing_in_db)}/{len(expected)}':<15} "
...
print()
for tname, cols in [("case_files", ["pdf_task_id", "pdf_generated_at"]),
