# Knowledge Graph Architecture & Implementation

**Scope:** Legal Knowledge Graph (Neo4j) architecture and implementation (2026-08-08 audit)
**Status:** ✅ ARCHITECTURE DEFINED - Implementation pending

## 1. Executive Summary

The NSA Webservice Legal Knowledge Graph (KG) is a multi-domain legal knowledge graph built in Neo4j Aura. It enables:

- **Legal instrument classification** (Act, Rule, Regulation, etc.)
- **Provision-level extraction** with canonical IDs and temporal models
- **Multi-domain support** (Food Safety, Environment, Municipal, etc.)
- **Cross-domain relationship mapping**
- **Interoperable with Qdrant vector store** for retrieval

**Current State:** ✅ Schema defined, implementation pending. Neo4j instance is live but empty.

## 2. Current State (What Exists)

### 2.1 Graph Database State (2026-08-10 Audit)

| Component      | Status                 | Details                                                        |
| -------------- | ---------------------- | -------------------------------------------------------------- |
| **Database**   | Neo4j Aura (5.27-aura) | Live, Bolt protocol over TLS                                   |
| **Connection** | ✅ Configured          | `NEO4J_URI`, `NEO4J_USERNAME`, `NEO4J_PASSWORD` in `.env`      |
...
### 2.2 Existing Neo4j Schema

### 2.2.1 Node Labels (9 — all created, all empty)
...
### 2.3 Existing Code Paths

### 2.3.1 `app/services/neo4j_graph.py` — Legacy case-file sync
...
- `build_cypher_payload()` — reads ALL `Entity`/`Relationship` rows from DB, returns JSON
- `setup_constraints_and_indexes()` — creates the 9 constraints + 3 indexes
...
**Critical limitation:** This pushes from the local `Entity`/`Relationship` tables (populated by `KnowledgeGraphEngine.build_graph_for_case()` from case-file data), NOT from legal corpus data. It is a case-file entity graph, not a legal-provision graph.

### 2.3.2 `neo4j_aura_loader.py` — Development bootstrap script
...
### 2.3.3 `app/knowledge_graph/` — Knowledge Graph extraction (Phase 14, case-file scope)
...
### 2.3.4 `app/rag/collections.py` — Multi-domain collection mapping
...
### 2.3.5 `app/rag/legal_sections.py` — Act section registries
...
### 2.4 Local Database State

### 2.4.1 LegalDocument table — 29 documents (ALL FSSAI)
...
### 2.4.2 LegalChunk table — 12,819 chunks
...
### 2.4.3 Entity table — 0 rows

### 2.4.4 Relationship table — 0 rows
...
### 2.6 Existing Ingestion Scripts
...
### 2.7 Existing Query/Retrieval Code
...
### 2.8 Missing Capabilities (vs. Task Requirements)
...
### 2.9.1 `push_to_neo4j()` destructively deletes all nodes
...
### 2.9.2 Neo4j labels are case-file entity labels, not legal-instrument labels
...
### 2.9.3 No `legal_domain` field anywhere
...
### 2.9.4 Existing tests depend on the current Neo4j schema
...
## 3. Target Architecture (Phase 1+ Implementation)
