...
def _chunk(
    ...
    payload = {
        ...
        "document_id": doc_id,
        "hierarchy_level": hl,
        "chunk_index": chunk_index or rank,
        "act_name": act_name,
    ...
    return CandidateItem(
        ...
        family=family,
        section=section,
        section_keys=list(sk),
        text=text,
        document_id=doc_id,
        hierarchy_level=hl,
        parent_key=parent_chunk_id,
        chunk_index=chunk_index or rank,
        authority="FSSAI",
        instrument_id=doc_id,
...
class TestBuildCandidates:
    ...
    def test_dedup(self, fake_payload_index, family_map):
        arm = {
            "arm": "F_test",
            "chunk_ids": ["chunk_1", "chunk_1", "chunk_2"],
            "kg_provisions": [],
            "latency_ms": 0,
            "error": None,
            "retriever": "r",
...
class TestMMR:
    ...
    def test_lambda_one_reduces_to_topk(self):
        cands = [
            _chunk(f"c{i}", score=1.0 - i * 0.1, rank=i + 1, text=f"unique text number {i} section") for i in range(5)
        ]
    ...
    def test_returns_k_items(self):
        cands = [_chunk(f"c{i}", score=0.9 - i * 0.05, rank=i + 1, text=f"text {i} section {i}") for i in range(10)]
        sel = MMRSelector().select(cands, 5)
        assert len(sel) == 5
    ...
    def test_preserves_section_subsection_chain(self):
        ...
        cands = [
            _chunk(
                ...
                section="31",
                hl=1,
                text="Section 31 adjudication of penalties",
                doc_id="doc1",
                chunk_index=1,
            ),
            _chunk(
                ...
                section="31",
                hl=2,
                text="Sub-section 31(2) penalty computation",
                doc_id="doc1",
                chunk_index=2,
            ),
            _chunk(
                ...
                section="31",
                hl=3,
                text="Clause 31(2)(a) fine amount",
                doc_id="doc1",
                chunk_index=3,
            ),
        ]
...
class TestCandidatesToArmResult:
    def test_round_trip(self):
        cands = [
            _chunk("c1", section="92"),
            _chunk("c2", section="55"),
            _kg("kg1", section="31"),
            _kg("kg2", section="82"),
        ]
...
class TestRegistry:
    def test_all_strategies_present(self):
        for name in ["V8_A_topk", "V8_B_mmr", "V8_C_legal_diversity", "V8_D_hierarchy", "V8_E_hybrid"]:
...
class TestDeterminism:
    ...
    def test_hybrid_deterministic(self):
        cands = [_chunk(f"c{i}", score=0.9 - i * 0.05, rank=i + 1, section=str(i)) for i in range(10)]
