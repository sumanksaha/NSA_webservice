...
class TestBackupCoordinator:
    def test_run_backup_all_succeed(self):
        ...
        with patch("app.services.sheets_sync.export_sheets_to_r2", return_value="r2:s.csv"), \
    ...
    def test_run_backup_all_fail(self):
        from app.services.backup_coordinator import run_backup
        with patch("app.services.sheets_sync.export_sheets_to_r2", return_value=None), \
    ...
    def test_run_backup_isolation(self):
        ...
        with patch("app.services.sheets_sync.export_sheets_to_r2", return_value="r2:s.csv"), \
             patch("app.services.airtable_sync.export_airtable_all_bases_to_r2", side_effect=Exception("down")), \
             patch("app.services.excel_sync.export_excel_to_r2", side_effect=lambda: (c.__setitem__("e", c["e"]+1), "r2:e.csv")[1]):
            r = run_backup()
...
class TestRestoreFromCsv:
    ...
    def test_download_failure_returns_zero(self):
        with patch("app.utils.sync._list_r2_csv_backups", return_value=["r2:ab.csv"]), \
    ...
    def test_excel_csv_success(self):
        ...
        with patch("app.utils.sync._list_r2_csv_backups", return_value=["r2:eb.csv"]), \
...
class TestSettingsBackupRoutes:
    ...
    def test_backup_redundant_post(self, client):
        with patch("app.services.backup_coordinator.run_backup", return_value={"sheets": True, "airtable": True, "excel": True, "r2_keys": []}):
...
class TestBackupScript:
    ...
    def test_script_delegates(self, app_ctx):
        ...
        try:
            ...
            with patch("app.services.backup_coordinator.run_backup", return_value={"sheets": True, "airtable": True, "excel": True}):
                r = mod.run_backup()
...
class TestRestoreFromSheetsCsv:
    ...
    def test_sheets_download_failure_returns_zero(self):
        with patch("app.utils.sync._list_r2_csv_backups", return_value=["r2:s.csv"]), \
             patch("app.utils.sync._download_r2_csv", return_value=None):
            from app.utils.sync import restore_from_sheets_csv
            assert restore_from_sheets_csv() == 0
...
class TestRestoreIfEmpty:
    ...
    def test_empty_restores_from_airtable(self):
        with patch("app.utils.sync._is_empty_sqlite_db", return_value=True), \
...
class TestTriggerBackup:
    def test_trigger_backup_delegates(self):
        with patch("app.services.backup_coordinator.run_backup", return_value={"sheets": True, "airtable": True, "excel": True}) as m:
