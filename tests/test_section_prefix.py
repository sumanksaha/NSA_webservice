...
class TestServeWiring:
    ...
    def test_retrieved_chunk_carries_clause_number(self):
        ...
        chunk = RetrievedChunk.from_dict({
            "chunk_id": "x", "score": 1.0, "text": "t",
            "section_number": None, "clause_number": "2.4.15",
        })
...
def _write_mining(tmp_path, monkeypatch):
    ...
    mining_file.write_text(
        json.dumps({
            ...
            "positives": [
                {"chunk_id": "pos-a", "text": "duties text", "rank": 1,
                 "gold_unit": "fssai:s16", "section": "16", "act_name": "FSS Act"},
            ],
            "negatives": [
                {"chunk_id": "neg-a", "text": "penalties text", "rank": 2, "tier": 3,
                 "score": 5.0, "features": {}, "section": "56", "act_name": "FSS Act"},
                {"chunk_id": "neg-b", "text": "powers text", "rank": 3, "tier": 2,
                 "score": 3.0, "features": {}, "section": "56", "act_name": "FSS Act"},
            ],
        }, ensure_ascii=False)
        + "\n",
        encoding="utf-8",
...
class TestDatasetIdentity:
    ...
    def test_payload_index_wins_over_mining_section(self, tmp_path, monkeypatch):
        ...
        pi.write_text(
            json.dumps({"id": "pos-a", "payload": {
                "section_number": "22", "clause_number": None, "act_name": "FSS Act"}})
            + "\n" + json.dumps({"id": "neg-a", "payload": {
                "section_number": None, "clause_number": "3.04", "act_name": "FSS Regs"}})
            + "\n",
            encoding="utf-8",
        )
    ...
    def test_section_prefix_bakes_identity(self, tmp_path, monkeypatch):
        pd = _write_mining(tmp_path, monkeypatch)
        monkeypatch.setenv("RAG_CE_SECTION_PREFIX", "true")
        examples = pd.build_pairwise_examples(
            mode="uniform", max_negatives_per_tier=8, section_prefix=True
        )
    ...
    def test_no_prefix_without_flag(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("RAG_CE_SECTION_PREFIX", "true")
        examples = pd.build_pairwise_examples(
            mode="uniform", max_negatives_per_tier=8, section_prefix=False
        )
...
class TestDomainBalance:
    def test_oversamples_underrepresented_domains(self):
        ...
        balanced = _balance_domains(examples, rng)
        from collections import Counter

        doms = Counter(
            str(e["gold_unit"]).split(":", 1)[0] for e in balanced
        )
        # All domains reach the largest domain's count (10)
    ...
    def test_preserves_existing_when_already_balanced(self):
        ...
        rng = random.Random(1)
        examples = (
            [{"gold_unit": "fssai:s16", "question_id": "Q1"} for _ in range(5)]
