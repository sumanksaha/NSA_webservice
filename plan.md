...
> **Status:** ✅ Phases 0–16, 20, 21, Phase A + OCR Phases B–E, Deepening D1–D5, S6a–d, S7, S2, S10a–c, S9a, Priority 6 infra, Priority 7, **Phase 15 ✅** (2026-08-22), **Phase 18 RBAC ✅** (44/44 pass, 2026-08-26), **Work Diary ✅** (28/28 pass, 2026-08-26), **Security close-out S10c+S2 ✅** (12/12 pass, 2026-08-26), **Redis/Celery ssl_cert_reqs fix ✅** (11/11 pass, 2026-08-26), **Case File Preview (TDD) ✅** (9 tests, 2026-08-26), **Adjudication Preview (TDD) ✅** (9 tests, 2026-08-26), and **Phase 6 deferred** all implemented & verified. **RAG Phase 1–5 ✅** (694 RAG tests). **Multi-Domain Phase 1 ✅** (37 tests, 2026-08-20). **P1-4 FSSAI re-ingest ✅** (15 tests, 2026-08-11). **Evaluation Framework ✅** (77 tests, 2026-08-12). **Benchmark v1.0 frozen ✅** (2026-08-12). **Rust PyO3 Normalizers ✅** (2026-08-12). **Remote Inference (Modal) ✅** (55 tests, 2026-08-16). **LangGraph Agent + M5 ✅** (75 tests, 2026-08-16). **FastAPI Gateway ✅** (50 tests, 2026-08-19). **Config seam ✅** (2026-08-22). Total: **~1,970 tests** (694 RAG + 57 ASGI + 46 CI/CD gates + 236 new since 2026-08-20). Phases 17, 19 ❌ pending. ENV-2/3/5/10 open.
...
| **18** | Multi-user RBAC — Role model, `@role_required`, comments, approval workflow    | ✅                                  | Role/UserRole/Comment models + migration + central `enforce_rbac` gate (`ROLE_BLUEPRINTS`) + `fso` role + record-level scoping (`scoped_officer_name()`/`case_visible_to_user()`) + `users.fso_name` binding + provisioning seam + admin Add User form + Comments API (`app/comments/`) + Notepad + flash rendering in `base.html`. `tests/test_rbac.py` — **44/44 pass** (2026-08-26)                                                                                                                                                                                            | `app/__init__.py`, `app/shared/rbac.py`, `app/comments/`, `app/auth/provisioning.py` |
| **Work Diary** | Per-FSO inspection diary + official report PDF                            | ✅                                  | `app/workdiary/` blueprint: per-FSO diary (Date / Place of Visit / Purpose / Activity), `Inspection.visit_purpose` column (migration `add_inspection_visit_purpose`), filterable by FSO/date-range/purpose, `/workdiary/preview` + `/workdiary/pdf` render official `FSO_Work_Diary_Template.html`. `tests/test_workdiary.py` — **28/28 pass** (2026-08-26)                                                              | `app/workdiary/`, `app/inspection/templates/inspection/index.html`                  |
| **Security close-out** | S10c backup monitoring + S2 CSP-report collector                          | ✅                                  | `run_backup()` bookkeeping to `settings` table; `GET /health/backups` dead-man's-switch; `POST /csp-report` collector (CSRF-exempt, both report formats, bounded 4 KB body). `tests/test_backup_monitoring.py` — **12/12 pass** (2026-08-26)                                                                                                                                                  | `app/services/backup_coordinator.py`, `app/tasks_webhook/routes.py`, `app/__init__.py` |
...
| 15   | Phase 18     | ✅ **Complete (2026-08-26)** — `app/shared/rbac.py` (`enforce_rbac` gate, `ROLE_BLUEPRINTS`), `scoped_officer_name()`/`case_visible_to_user()` record-level scoping across CaseFile/Adjudication/Inspection/Work Diary/Comments; `users.fso_name` binding + `app/auth/provisioning.py` + Comments API (`app/comments/`) + Notepad; `tests/test_rbac.py` — **44/44 pass**. | `app/__init__.py`, `app/shared/rbac.py`, `app/comments/`, `app/auth/provisioning.py` |
...
> **📌 Next 3 steps (highest future impact, smallest effort):** **Phase 17** (Supabase bridge + conflict resolution + sync-status UI in `app/sync/`; R2/B2/Cloudinary/Sheets/Airtable/Excel sync done), **Phase 19** (AI Case Intelligence — evidence strength + readiness score; not started), **RAG Query Interface UI** (task.md §7 — backend done, frontend form/template/JS/nav-link + 8 tests still planned). Plus: Pre-commit hook stack fixed (`.pre-commit-config.yaml` hardened: mypy non-blocking `|| true`, pytest fast-subset filter, ce-v2-gate file-scoped, `minimum_pre_commit_version` pin; 5 new `TestPreCommitConfig` tests). CI/CD gates G1–G14 all ✅ done (see §5).
...
| --- | ---------------------------------------- | ---------------------------------- | ------ | ----------------------------------- |
| 1   | Dev/test–production DB parity (was: "SQLite for production" — prod has run Postgres via `DATABASE_URL` since Priority 6) | MEDIUM (Postgres-specific behavior untested locally) | Medium | docker-compose local Postgres for dev + point conftest at it (CI `test-postgres` job already exists) |
