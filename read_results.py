...
for mname, m in data["retrieval_diagnostics"].items():
    print(
        f"{mname}: R@1={m['R@1']}, R@20={m['R@20']}, R@50={m['R@50']}, "
...
for key, m in data["llm_generation"]["aggregate"].items():
    print(
        f"{key}: n={m.get('n')}, ans_correct={m.get('answer_correctness_avg')}, "
...
if "per_question" in lg:
    ...
    for entry in lg["per_question"][:3]:
        print(
            f"  {entry['question_id']}_{entry['model']}_{entry['mode']}: "
...
for item in data.get("improvement_ranking", []):
    ...
    print(
        f"    failure_pct={item.get('failure_pct')}, current={item.get('current_R@20', item.get('current_recall10', item.get('current_recall', 'N/A')))}"
    )
